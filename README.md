...
| `must.html` | ⭐ **みんなが行きたい** — `must.json` の希望スポットを人気順＋地図で表示 |
...
ヘッダーの「🗺️ 地図 / 📍 スポット集 / ⭐ 行きたい / 🇺🇸 州別 / 🎪 催し / 🍴 グルメ / 📚 Wiki」で行き来できます。

`spots.html` のデータは `data/spots.json` で管理（カテゴリ・座標・説明を編集するだけ）。
ルートに採用したいスポットを `data/itinerary.json` に転記すれば地図トップに反映されます。
**みんなが行きたいスポット**は専用ページ `must.html`（データは `data/must.json`）。
`spots.json` と同じ `name`（必要なら `state`）と行きたい人の名前 `by`、一言 `note` を
書いて push するだけ。座標・カテゴリ・説明は `spots.json` から自動で紐付き、★の数（＝
行きたい人の数）が多い順に地図＋リスト表示されます。
